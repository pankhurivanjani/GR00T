...
class IRLSinglePandaGripperColorTaskDataConfig(BaseDataConfig):
    ...
    def transform(self):
        ...
        transforms = [
            ...
            VideoCrop(apply_to=self.video_keys, scale=0.95),
            # VideoResize(apply_to=self.video_keys, height=224, width=224, interpolation="linear"),
            VideoResize(apply_to=self.video_keys, height=224, width=224, interpolation="nearest", antialias=False), # 保留纯色块的边界, 关闭抗锯齿让色块边缘保持原始锐度
            # VideoColorJitter(
            #     apply_to=self.video_keys,
            #     brightness=0.3,
            #     contrast=0.4,
...
DATA_CONFIG_MAP = {
    ...
    "irl_panda_color_task": IRLSinglePandaGripperColorTaskDataConfig(),
}
